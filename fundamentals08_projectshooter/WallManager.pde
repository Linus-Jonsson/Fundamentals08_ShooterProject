    ...
    for (int i=0; i<nPieces/4; i++) {
      for (int j=0; j<nPieces/5; j++)
        if (i*4+j == 0) {
          ...
          wall[i*4+j] = new WallPiece(pos.x+wallPieceDiameter*2.5*i, pos.y+wallPieceDiameter*j-wallPieceDiameter, new BoundingCircle(0, 0, wallPieceDiameter), wallPieceDiameter);
        } else if (i*4+j == 15) {
